...
class ChuckService {
  ...
  Future<ChuckResponse> getJokeRandom() async {
    Dio dio = Dio(options);
    try {
      Response response = await dio.get(apiBaseUrl + '/random');
      if (response.statusCode == 200 || response.statusCode == 201) {
        debugPrint(response.data.toString());
        debugPrint(json.encode(response.data));
        ChuckResponse responseJson = ChuckResponse.fromMap(response.data);
        return responseJson;
      }
  ...
  Future<ChuckResponse> getJokeRandomByCategory(String category) async {
    ...
    try {
      ...
      Response response =
          await dio.get(apiBaseUrl + '/random?category=' + category);
      if (response.statusCode == 200 || response.statusCode == 201) {
        debugPrint(response.data.toString());
        debugPrint(json.encode(response.data));
        ChuckResponse responseJson = ChuckResponse.fromMap(response.data);
